...
| 42 | 08-15 | **v6 series: fix (1/2) + kselftest (2/2), independently applicable.** Per request, test is its own commit. 2/2 = tools/testing/selftests/microblaze/entry_abi.c: raw syscall reads r1 before/after brki trap, kselftest TAP harness, arch-gated (#ifdef __microblaze__ else skip). Verified: 'ok 1 user stack pointer preserved across 100000 syscalls' pass:1 rc=0 on fixed kernel. Both patches dry-run apply clean (2/2 fwd, 1/2 reverse vs fixed tree). Series in patches/linux-v6/ (cover + 1/2 + 2/2). NOT sent -- awaiting go. Compiles against real kselftest.h | QEMU boot TAP + dry-run apply | tca 86fbc6e |
| 43 | 08-15 | **Answered 'did you verify the test catches the bug' -- BOTH sides now verified.** BEFORE (this was the gap): booted stock-entry.S kernel with init=/usr/bin/entry_abi; the test's own faccessat(AT_FDCWD) syscall corrupts saved SP -> fault dump r1=0xFFFFFF9C = AT_FDCWD (the arg itself), process dies. So test is PROVABLY sensitive (no false pass). AFTER: ok 1 pass. Honest limit recorded: bug is total-boot-failure, so unpatched kernel can't reach userspace to print graceful 'not ok' -- signal is crash/no-ok = CI fail. Corrected earlier overclaim. Fork-based variant tried, didn't help (PID1 crash still panics). Docs+cover letter fixed | init=entry_abi A/B on both kernels | tcgcov 630f38f |
| 44 | 08-15 | **Emailed Neal** (neal.frager@amd.com, cc self): personal note that the bot coded+tested a kselftest in case we want one, + coverage summary (14/14 reachable entry.S reserve sites executed; 15th kgdb-gated absent), + the before/after test verification (r1=AT_FDCWD proof), + v4/v5 both boot hardware. Attached v6 1/2 (fix) + 2/2 (test). msmtp exit 0. Nothing sent to a list yet. Copy: patches/linux-v6/neal-note-SENT.eml | -- | reported by Sam |
| 45 | 08-15 | **Sent Neal the git-am-able v6 series** (threaded: cover 0/2 + 1/2 fix + 2/2 test), Message-Id 20260816000339.69325-1. Follow-up to the row-44 note (which had the patches as attachments); this is applyable. Threading via In-Reply-To on cover; suppress-cc=all so the fix's Cc:stable trailer did NOT leak to stable@ (Neal + self only). Fixed 2/2 subject numbering + real diffstat first. Still NOT sent to any list | git send-email 3x Result OK | -- |
| 46 | 08-15 | **Attempted broad LTP on the fixed kernel; blocked by the initramfs-only rootfs.** Built LTP for microblaze (2772 bins), full corpus = 475MB initramfs -> 196MB image, never reached banner (0B, 100%cpu, 5+min). Pruned to 326 entry-path bins (fork/exec/wait/signal/ptrace/mmap/timer/faccessat) = 90MB/39MB image, STILL 0B in 4+min while base 9.7MB image banners in ~2s (confirmed same launch method). Root cause: petalogix-s3adsp1800 has no block device (pflash only), so rootfs must be in-RAM initramfs; a large test tree can't boot under TCG. Broad LTP needs a disk/NFS-root target or real HW. Fixed kernel already validated broadly: shell boot, mb-abi-stress (60k SP-checked syscalls + fork/ptrace/signal/fault/brk) CANARY PASS, entry_abi ok 1, 14/14 sites covered | small vs slim boot A/B | LTP img in brout |
| 47 | 08-16 | **Neal's tst-cancel/tst-eintr concern investigated -- NOT our patch (A/B proven).** Built cancel-diag (isolates each cancellation/EINTR/restart mechanism in a forked child w/ timeout). PATCHED kernel: nanosleep-cancel PASS, read()-cancel HANG, async-cancel(read) HANG, NOTHING crashes (idle-deadlock, not fault). Built BASELINE = workaround-GCC + STOCK entry.S (no Linux patch, but boots): **IDENTICAL results** -- read()-cancel HANG on both. So the cancellation hang is pre-existing glibc/microblaze, present with AND without our patch. Logic also: our patch touches the common signal/do_notify_resume path that nanosleep-cancel proves healthy; our bug CRASHES (SP corruption) not hangs. tst-eintr1 'sometimes segfault' = intermittent race, same family. Volume now baseline state (workaround GCC+stock entry.S) -- restore v5 for future work | cancel-diag A/B on both kernels | draft reply to Neal pending Sam's go |
| 48 | 08-16 | **Root-caused the cancellation bug deep (separate from our patch).** Traced facts: cancel of thread blocked in read() HANGS (deferred AND async); cancel of thread in nanosleep genuinely CANCELS (traced -> CANCELED). glibc 2.44 microblaze syscall_cancel.S is hand-written; its pc-check labels+extraction are CORRECT (probe: SA_RESTART signal interrupting read leaves pc=brki=0x..399c, IN [start,end)). SIGCANCEL uses SA_RESTART so a blocked syscall relies entirely on cancellation_pc_check OR async. Handler DOES run for read-blocked thread (probe fired). Yet read cancel hangs both modes while nanosleep works -- same bridge. Points to read-family-specific issue past pc-check (delivery/restart/unwind asymmetry), NOT our entry.S. Reproducers: cancel-min/both/async/pc-probe in scratchpad. Next: microblaze gdb on guest to pin exact line, or file glibc/kernel bug | 5 traced reproducers, A/B on both kernels | separate bug, not v6 |
